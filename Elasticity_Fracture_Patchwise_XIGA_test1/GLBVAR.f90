...
!   GLOBAL PARAMETER
	...
	INTEGER, PARAMETER :: NUM_IGA_PATCH = 3	! number of IGA patches
	INTEGER, PARAMETER :: NUM_EGM_PATCH = 1	! number of EGM patches
	INTEGER, PARAMETER :: NUM_PATCH = NUM_IGA_PATCH + NUM_EGM_PATCH	! Total number of patches

	INTEGER :: IGA_ORDER(2), EGM_ORDER(2), IGA_KNOTS(2), EGM_KNOTS(2), IGA_REGULARITY(2), EGM_ETA_ORDER
	INTEGER :: NUMBS(NUM_PATCH,2), DOF, LOCAL_DOF(NUM_PATCH), BD_DOF, TS_NUMBS

	INTEGER, PARAMETER :: NM_NUMBD = 6, DR_NUMBD = NUM_PATCH
	TYPE(BD_INFO) :: NM_BDNDX(NM_NUMBD,MAX_LENGTH), DR_BDNDX(DR_NUMBD,MAX_LENGTH), ZERO_BDNDX(DR_NUMBD,MAX_LENGTH)
	...
	TYPE(KNOT_VECTOR), SAVE :: GEO_KVEC(NUM_PATCH,2), IGA_KVEC(NUM_IGA_PATCH,2), EGM_KVEC(NUM_EGM_PATCH,2), BASIS_KVEC(NUM_PATCH,2)
	TYPE(CONTROL_POINTS_2D) :: GEO_CTL(NUM_PATCH), IGA_CTL(NUM_IGA_PATCH)
	...
	INTEGER :: NUMGSPT, LN_NUMGSPT, MX_GSPT
	INTEGER, PARAMETER :: L2_GSPT = 12
	INTEGER :: XIGA_NUMGSPT
	REAL*8 :: IR_GRID(NUM_PATCH, 2, MAX_LENGTH)
	INTEGER :: NUMIR(NUM_PATCH,2), NUM_IRBOX(2), NUM_LN

	REAL*8, ALLOCATABLE :: GSX(:), GSXW(:), GSY(:), GSYW(:)
	REAL*8, ALLOCATABLE :: MX_GSX(:), MX_GSXW(:), MX_GSY(:), MX_GSYW(:)
	REAL*8 :: L2_GSX(L2_GSPT), L2_GSXW(L2_GSPT), L2_GSY(L2_GSPT), L2_GSYW(L2_GSPT)
	REAL*8, ALLOCATABLE :: XIGA_GSX(:), XIGA_GSY(:), XIGA_GSXW(:), XIGA_GSYW(:)
...
!	MATERIAL COEFFICIENTS
	...
	REAL*8, PARAMETER :: Q = -DCOS((0.5444837370D0 - 1.D0)*135.D0*DEGREE)/DCOS((0.5444837370D0 + 1.D0)*135.D0*DEGREE)
...
!		Single - Edged - Cracked Stress Intensity Factor
	...
	REAL*8, PARAMETER :: INNER_RADIUS = 0.5D0
	REAL*8, PARAMETER :: INNER_LAYER = 0.1D0
