...
! 	IF ((PT%X - R(1)%X).GE.TOL .AND. (PT%X - R(2)%X).LE.TOL .AND. (PT%Y - R(1)%Y).GE.TOL .AND. (PT%Y - R(4)%Y).LE.TOL) THEN
! 		BELONGTO_R = .TRUE.
! 	ELSE
! 		BELONGTO_R = .FALSE.
! 	ENDIF
...
END FUNCTION PTLIEON
	...
	TYPE(MATRIX_22) FUNCTION INVERSE_MATRIX_22(MAT)
		...
		DO I=1, 2
			DO J=1, 2
				IF (DABS(DMY_MAT%ENT(I,J)).LT.TOLERANCE) THEN
		...
		IF (DABS(DET_M).GE.TOLERANCE) THEN
...
! 	ELSE
		IF (DABS(R).LE.TOLERANCE) THEN
			THETA = 0.0D0
		ELSEIF (X.GE.TOLERANCE.AND.DABS(Y).LE.TOLERANCE.AND.DABS(R).GT.TOLERANCE) THEN
			THETA = 0.0D0
	! 	ELSEIF (X.LT.-TOLERANCE.AND.DABS(Y).LE.TOLERANCE.AND.DABS(R).GT.TOLERANCE) THEN
	! 		THETA = DACOS(-1.D0)
		ELSEIF (Y.GT.TOLERANCE.AND.DABS(X).LE.TOLERANCE.AND.DABS(R).GT.TOLERANCE) THEN
