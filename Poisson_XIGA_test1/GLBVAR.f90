...
!   GLOBAL PARAMETER
	...
	INTEGER :: IGA_ORDER(2), EGM_ORDER(2), EXTRA_KNOTS(2)
	INTEGER :: NUMBS(NUM_PATCH,2), DOF, LOCAL_DOF(NUM_PATCH), BD_DOF, TEST_NUMBS
	TYPE(BD_INFO) :: BDNDX(NUM_PATCH,MAX_LENGTH)
	LOGICAL, ALLOCATABLE :: BD_MASK(:)
	
	!!  VARIABLES FOR GEOMETRIC MAP
	TYPE(KNOT_VECTOR), SAVE :: GEO_KVEC(NUM_PATCH,2), IGA_KVEC(2), EGM_KVEC(2), BASIS_KVEC(NUM_PATCH,2)
	TYPE(CONTROL_POINTS_2D) :: GEO_CTL(NUM_PATCH), EGM_CTL, IGA_CTL
