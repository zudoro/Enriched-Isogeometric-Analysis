...
SUBROUTINE IMPOSEBD(STIF_K, STIF_F)

	REAL*8, INTENT(INOUT) :: STIF_K(DOF,DOF), STIF_F(DOF)
	...
	DO I=1, DOF
		DO J=1, DOF
			IF (DABS(STIF_K(I,J)).LE.EPS) THEN
				STIF_K(I,J) = 0.D0
			ENDIF
		ENDDO
	ENDDO
...
! 			STIF_F(NEUMANN_BDNDX(J,I)%GL_NDX) = STIF_F(NEUMANN_BDNDX(J,I)%GL_NDX) + LN_INT%X
! 			STIF_F(DOF + NEUMANN_BDNDX(J,I)%GL_NDX) = STIF_F(DOF + NEUMANN_BDNDX(J,I)%GL_NDX) + LN_INT%Y
! 		ENDDO
! 	ENDDO
! 
...
! 
! 	DO J=1, 2*DOF
! 		STIF_F(J) = STIF_F(J) - PINCH_DISP(1)%X*STIF_K(J,LOCAL_DOSTIF_F(1)-NUMBS(1)+1) - PINCH_DISP(1)%Y*STIF_K(J, DOF + LOCAL_DOSTIF_F(1)-NUMBS(1)+1)
! 		STIF_F(J) = STIF_F(J) - PINCH_DISP(2)%X*STIF_K(J,DOF) - PINCH_DISP(2)%Y*STIF_K(J, 2*DOF)
! 	ENDDO
! 
! 	STIF_F(LOCAL_DOSTIF_F(1)-NUMBS(1)+1) = PINCH_DISP(1)%X; STIF_F(DOF) = PINCH_DISP(2)%X
! 	STIF_F(DOF + LOCAL_DOSTIF_F(1)-NUMBS(1)+1) = PINCH_DISP(1)%Y; STIF_F(2*DOF) = PINCH_DISP(2)%Y
! 	DO J=1, 2*DOF
! 		STIF_K(LOCAL_DOSTIF_F(1)-NUMBS(1)+1,J) = 0.0D0; STIF_K(DOF,J) = 0.D0
! 		STIF_K(DOF + LOCAL_DOSTIF_F(1)-NUMBS(1)+1,J) = 0.0D0; STIF_K(2*DOF, J) = 0.D0
! 
! 		STIF_K(J,LOCAL_DOSTIF_F(1)-NUMBS(1)+1) = 0.0D0; STIF_K(J,DOF) = 0.D0
! 		STIF_K(J,DOF + LOCAL_DOSTIF_F(1)-NUMBS(1)+1) = 0.0D0; STIF_K(J,2*DOF) = 0.D0
! 	ENDDO
! 	STIF_K(LOCAL_DOSTIF_F(1)-NUMBS(1)+1, LOCAL_DOSTIF_F(1)-NUMBS(1)+1) = 1.0D0; STIF_K(DOF,DOF) = 1.D0
! 	STIF_K(DOF + LOCAL_DOSTIF_F(1)-NUMBS(1)+1, DOF + LOCAL_DOSTIF_F(1)-NUMBS(1)+1) = 1.0D0; STIF_K(2*DOF,2*DOF) = 1.D0
! 
! 	PRINT*, 
...
! 					PRINT*, LC_NUM_SUM
		DO I=1, BDNDX(II,1)%LC_NUM
			DO J=1, DOF
				STIF_F(J) = STIF_F(J) - SUB_F(LC_NUM_SUM + I)%X*STIF_K(J, BDNDX(II,I)%GL_NDX)
			ENDDO
		ENDDO
	ENDDO
! PRINT*, 'HERE 4'
	DO II=1, NUM_PATCH
		...
		ENDIF
		DO I=1, BDNDX(II,1)%LC_NUM
			STIF_F(BDNDX(II,I)%GL_NDX) = SUB_F(LC_NUM_SUM + I)%X
! 			PRINT*, STIF_F(BDNDX(II,I)%GL_NDX), STIF_F(DOF+BDNDX(II,I)%GL_NDX)
			DO J=1, DOF
				STIF_K(BDNDX(II,I)%GL_NDX,J) = 0.0D0
				STIF_K(J,BDNDX(II,I)%GL_NDX) = 0.0D0
			ENDDO
			STIF_K(BDNDX(II,I)%GL_NDX, BDNDX(II,I)%GL_NDX) = 1.0D0
		ENDDO
	ENDDO
	
! 	DO II = LOCAL_DOF(1) + 1, LOCAL_DOF(1) + NUMBS(2,1)
! 		DO JJ = 1, DOF
! 			STIF_K(II,JJ) = 0.D0
! 			STIF_K(JJ,II) = 0.D0
! 		ENDDO
