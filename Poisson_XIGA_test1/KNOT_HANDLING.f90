...
CONTAINS
	...
	!!  INSERT KNOT VALUES. DOES NOT CHANGE THE POLYNOMIAL ORDER
	TYPE(KNOT_VECTOR) FUNCTION DEGREE_ELEVATION_KVEC(KVEC,REGULARITY_OPTION)
		...
		IF (INCREASE_REGULARITY) THEN
			...
			DEGREE_ELEVATION_KVEC = ARRAY_TO_KNOT(POLY_ORDER+1,NUM_KNOTS,KNOTS,MULTIPLICITIES)
		ELSE
			DO K = 1, NUM_KNOTS
				MULTIPLICITIES(K) = MULTIPLICITIES(K) + 1
			ENDDO
	...
	END FUNCTION DEGREE_ELEVATION_KVEC
	...
	!! FIND CONTROL POINTS AFTER DEGREE ELEVATION
	TYPE(CONTROL_POINTS_2D) FUNCTION DEGREE_ELEVATION_CTL(OLD_CTL,GEO_NUMBS,CURRENT_DEG,ELEVATED_DEG)
	
		TYPE(CONTROL_POINTS_2D), INTENT(IN) :: OLD_CTL
		INTEGER, INTENT(IN) :: GEO_NUMBS(2), CURRENT_DEG(2), ELEVATED_DEG(2)
		
		TYPE(CONTROL_POINTS_2D) :: NEW_CTL
		REAL*8 :: C(3)
		TYPE(POINT2D), ALLOCATABLE :: NEW_CTL_ROW(:,:), NEW_CTL_COLM(:,:)
		INTEGER :: I, J, K1, K2, T(2), P(2)
		
! 		PRINT*, 'HERE-1'
		
		P = CURRENT_DEG
		T(1) = ELEVATED_DEG(1) - CURRENT_DEG(1)
		T(2) = ELEVATED_DEG(2) - CURRENT_DEG(2)
		
! 		PRINT*, P, T

		ALLOCATE(NEW_CTL_ROW(0:GEO_NUMBS(2)-1,0:P(1)+T(1)), NEW_CTL_COLM(0:P(1)+T(1),0:P(2)+T(2)))
		
		IF (T(1).LT.0 .OR. T(2).LT.0) THEN
			PRINT*, 'ERROR - DEGREE_ELEVATION_CTL : INPUT ELEVATED DEG IS WRONG'
			STOP
		ENDIF

! 		PRINT*, 'HERE0'
		
		!! DEGREE ELEVATION ALONG XI - DIRECTION
		DO K1 = 0, GEO_NUMBS(2)-1
			DO I = 0, P(1)+T(1)
				NEW_CTL_ROW(K1,I) = POINT2D(0.D0,0.D0)
				DO J = MAX(0, INT(I-T(1))), MIN(P(1),I)
! 					PRINT*, 'S1', P(1), I
					C(1) = COMBINATION(P(1),J)
! 					PRINT*, 'S2', T(1), INT(I-J)
					C(2) = COMBINATION(T(1),INT(I-J))
! 					PRINT*, 'S3', T(1), INT(I-J)
					C(3) = COMBINATION(P(1)+T(1),I)
					NEW_CTL_ROW(K1,I) = NEW_CTL_ROW(K1,I) + POINT2D(C(1)*C(2)*OLD_CTL%PTS(J,K1,0)%X/C(3), C(1)*C(2)*OLD_CTL%PTS(J,K1,0)%Y/C(3))
				ENDDO
			ENDDO
		ENDDO
...
! 		PRINT*, 'HERE1'
		...
		DO K1 = 0, GEO_NUMBS(2)-1
			DO I = 0, P(1)+T(1)
				NEW_CTL%PTS(I,K1,0) = NEW_CTL_ROW(K1,I)
			ENDDO
		ENDDO
		
		!! DEGREE ELEVATION ALONG ETA - DIRECTION
		DO K2 = 0, P(1)+T(1)
			DO I = 0, P(2)+T(2)
				NEW_CTL_COLM(K2,I) = POINT2D(0.D0, 0.D0)
				DO J = MAX(0, INT(I-T(2))), MIN(P(2),I)
					C(1) = COMBINATION(P(2),J)
					C(2) = COMBINATION(T(2),INT(I-J))
					C(3) = COMBINATION(P(2)+T(2),I)
					NEW_CTL_COLM(K2,I) = NEW_CTL_COLM(K2,I) + POINT2D(C(1)*C(2)*NEW_CTL%PTS(K2,J,0)%X/C(3), C(1)*(2)*NEW_CTL%PTS(K2,J,0)%Y/C(3))
! 					PRINT*, I, J, C, NEW_CTLY(K2,I)
				ENDDO
			ENDDO
		ENDDO
...
! 		PRINT*, 'HERE2'
		...
		DO K2 = 0, P(1)+T(1)
			DO J = 0, P(2)+T(2)
				NEW_CTL%PTS(K2,J,0) = NEW_CTL_COLM(K2,J)
			ENDDO
		ENDDO
		
! 		PRINT*, 'HERE3'
		
		DEGREE_ELEVATION_CTL = NEW_CTL
		
	END FUNCTION DEGREE_ELEVATION_CTL
