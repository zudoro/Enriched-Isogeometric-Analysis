...
! 		ELSE
! 			IF (PHY_PT%X.LE.EPS) THEN
! 				OLD_PT = POINT2D(0.25D0,0.5D0)
! 			ELSE
! 				OLD_PT = POINT2D(0.75D0,0.5D0)
! 			ENDIF
! 		ENDIF

! 		IF (DABS(OLD_PT%X).LE.EPS) THEN
! 			OLD_PT%X = 0.D0
! 		ENDIF
! 
! 		IF (DABS(OLD_PT%Y).LE.EPS) THEN
! 			OLD_PT%Y = 0.D0
! 		ENDIF
		
		IF (PRESENT(SIDE) .AND. PATCH==1) THEN
			IF (PATCH.NE.1) THEN
				PRINT*, 'ERROR - GET_REF_PT : APPLIED SIDE OPTION WRONG ! '
				STOP
			ENDIF
			OLD_PT%X = 1.D0
			IF (SIDE.EQ.'L') THEN
...
! 				PRINT*, 'GET_REF_PT-HERE2'
				...
				IF (PRESENT(SIDE) .AND. PATCH==1) THEN
					...
					ENDIF
					IF (SIDE.EQ.'L') THEN
						IF (OLD_PT%Y.LE.0.D0 .OR. OLD_PT%Y.GE.0.5D0) THEN
							OLD_PT%Y = 0.001D0*K(2)
							GOTO 138
						ENDIF
					ELSEIF (SIDE.EQ.'R') THEN
						IF (OLD_PT%Y.LE.0.5D0 .OR. OLD_PT%Y.GE.1.D0) THEN
							OLD_PT%Y = 0.5D0 + 0.001D0*K(2)
							GOTO 138
						ENDIF
					ENDIF
				ENDIF
				...
				IF (OLD_PT%Y.LE.0.D0 .OR. OLD_PT%Y.GE.1.D0) THEN
					K(2) = K(2) + 1
					IF (PATCH==1) THEN
						IF (PHY_PT%Y.LE.EPS) THEN
							OLD_PT%Y = 0.01D0*K(2)
						ELSE
							OLD_PT%Y = 0.5D0 + 0.01D0*K(2)
						ENDIF
					ELSEIF (PATCH==2) THEN
						OLD_PT%Y = 0.01D0*K(2)
					ENDIF
...
! 		ENDIF
	...
	REAL*8 FUNCTION GET_DET_LN(REF_PT, GAMMA, PATCH)
		...
		IF (GAMMA.EQ.1 .OR. GAMMA.EQ.5) THEN
