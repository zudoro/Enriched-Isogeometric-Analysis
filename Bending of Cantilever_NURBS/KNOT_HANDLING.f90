...
CONTAINS
	...
	!!  INSERT KNOT VALUES. DOES NOT CHANGE THE POLYNOMIAL ORDER
	TYPE(KNOT_VECTOR) FUNCTION END_KNOT_INSERTION(KVEC, NUM_INSERTION, WHICH_SIDE, REGULARITY_OPTION)

		TYPE(KNOT_VECTOR), INTENT(IN) :: KVEC
		INTEGER, INTENT(IN) :: NUM_INSERTION
		CHARACTER(LEN=1), INTENT(IN) :: WHICH_SIDE ! L = LEFT, R = RIGHT, B = BOTH
		INTEGER, OPTIONAL, INTENT(IN) :: REGULARITY_OPTION(2)

		REAL(8) :: KNOTS(MAX_LENGTH), NEW_KNOTS(MAX_LENGTH)
		INTEGER :: POLY_ORDER, MULTIPLICITIES(MAX_LENGTH), NEW_MULTIPLICITIES(MAX_LENGTH), NUM_KNOTS

		IF (WHICH_SIDE.EQ.'L') THEN
			CALL KNOT_TO_ARRAY(KVEC,POLY_ORDER,NUM_KNOTS,KNOTS,MULTIPLICITIES)
			NEW_KNOTS(1) = KNOTS(1)
			NEW_MULTIPLICITIES(1) = MULTIPLICITIES(1)
			...
			DO K = 2*(NUM_INSERTION+1)-NUM_INSERTION, NUM_KNOTS + NUM_INSERTION
				NEW_KNOTS(K) = KNOTS(K-NUM_INSERTION)
				NEW_MULTIPLICITIES(K) = MULTIPLICITIES(K-NUM_INSERTION)
			ENDDO
			
			END_KNOT_INSERTION = ARRAY_TO_KNOT(POLY_ORDER,NUM_KNOTS+NUM_INSERTION,NEW_KNOTS,NEW_MULTIPLICITIES)
		
		ELSEIF (WHICH_SIDE.EQ.'R') THEN
			
			CALL KNOT_TO_ARRAY(KVEC,POLY_ORDER,NUM_KNOTS,KNOTS,MULTIPLICITIES)
			
			DO K = 1, NUM_KNOTS-1
				NEW_KNOTS(K) = KNOTS(K)
				NEW_MULTIPLICITIES(K) = MULTIPLICITIES(K)
			ENDDO
			
			K = NUM_KNOTS-1
			DO J = 1, NUM_INSERTION
				INDX = K+J
				NEW_KNOTS(INDX) = (1.0D0*J)*(KNOTS(K+1)-KNOTS(K))/(NUM_INSERTION+1) + KNOTS(K)
				NEW_MULTIPLICITIES(INDX) = 1
			ENDDO
			NEW_KNOTS(NUM_KNOTS+NUM_INSERTION) = KNOTS(NUM_KNOTS)
			NEW_MULTIPLICITIES(NUM_KNOTS+NUM_INSERTION) = MULTIPLICITIES(NUM_KNOTS)
			
			END_KNOT_INSERTION = ARRAY_TO_KNOT(POLY_ORDER,NUM_KNOTS+NUM_INSERTION,NEW_KNOTS,NEW_MULTIPLICITIES)
		
		ELSEIF (WHICH_SIDE.EQ.'B') THEN
			CALL KNOT_TO_ARRAY(KVEC,POLY_ORDER,NUM_KNOTS,KNOTS,MULTIPLICITIES)
			NEW_KNOTS(1) = KNOTS(1)
			NEW_MULTIPLICITIES(1) = MULTIPLICITIES(1)
			...
			DO K = 2*(NUM_INSERTION+1)-NUM_INSERTION, NUM_KNOTS + NUM_INSERTION - 1
				NEW_KNOTS(K) = KNOTS(K-NUM_INSERTION)
				NEW_MULTIPLICITIES(K) = MULTIPLICITIES(K-NUM_INSERTION)
			ENDDO
			
			K = NUM_KNOTS + NUM_INSERTION - 1
			DO J = 1, NUM_INSERTION
				INDX = K+J
				NEW_KNOTS(INDX) = (1.0D0*J)*(KNOTS(K-NUM_INSERTION+1)-KNOTS(K-NUM_INSERTION))/(NUM_INSERTION+1) + KNOTS(K-NUM_INSERTION)
				NEW_MULTIPLICITIES(INDX) = 1
			ENDDO
			NEW_KNOTS(NUM_KNOTS+2*NUM_INSERTION) = KNOTS(NUM_KNOTS)
			NEW_MULTIPLICITIES(NUM_KNOTS+2*NUM_INSERTION) = MULTIPLICITIES(NUM_KNOTS)
			
			END_KNOT_INSERTION = ARRAY_TO_KNOT(POLY_ORDER,NUM_KNOTS+2*NUM_INSERTION,NEW_KNOTS,NEW_MULTIPLICITIES)
			
		ENDIF

	END FUNCTION END_KNOT_INSERTION
