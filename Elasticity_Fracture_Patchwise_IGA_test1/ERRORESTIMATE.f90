		...
		DO KK=1, UBOUND(INTERFACE_NDX,1)
			DO I=1, UBOUND(IGA_INDX,2)
				DO II=1, UBOUND(INTERFACE_NDX(1)%SLAVE_PATCH_NDX(:),1)
					IF (INTERFACE_NDX(KK)%MASTER_PATCH_NDX.EQ.JJ .AND. INTERFACE_NDX(KK)%SLAVE_PATCH_NDX(II).EQ.PATCH .AND. &
							INTERFACE_NDX(KK)%LC_NDX(2*II+1).EQ.IGA_INDX(PATCH,I)%A .AND. INTERFACE_NDX(KK)%LC_NDX(2*(II+1)).EQ.IGA_INDX(PATCH,I)%B) THEN
						SF(INTERFACE_NDX(KK)%GL_NDX) = LOCAL_IGA_SF(PATCH,I)
						GOTO 3
					ENDIF
				ENDDO
				3 CONTINUE
