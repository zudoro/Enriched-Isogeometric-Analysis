...
SUBROUTINE GET_GEO()
	...
	TS_KVEC(1:2)%POLY_ORDER = 2          
	TS_KVEC(1:2)%LENGTH = 5
	TS_NUMBS = 3
	TS_KVEC(1)%KNOTS(0:TS_KVEC(1)%LENGTH) = (/0.D0, 0.D0, 0.D0, 1.D0, 1.D0, 1.D0/)
	TS_KVEC(2)%KNOTS(0:TS_KVEC(2)%LENGTH) = (/0.D0, 0.D0, 0.D0, 1.D0, 1.D0, 1.D0/)
	
	TS_CTL%D = 2
	TS_CTL%PTS(0,0,0) = POINT2D(0.D0, -1.D0)
	TS_CTL%PTS(1,0,0) = POINT2D(1.D0,-1.D0)
	TS_CTL%PTS(2,0,0) = POINT2D(1.D0,0.D0)
	TS_CTL%PTS(0,1,0) = POINT2D(-1.D0,-1.D0)
	TS_CTL%PTS(1,1,0) = POINT2D(0.D0,0.D0)
	TS_CTL%PTS(2,1,0) = POINT2D(1.D0,1.D0)
	TS_CTL%PTS(0,2,0) = POINT2D(-1.D0,0.D0)
	TS_CTL%PTS(1,2,0) = POINT2D(-1.D0,1.D0)
	TS_CTL%PTS(2,2,0) = POINT2D(0.D0,1.D0)
	
	DO I = 0, TS_NUMBS - 1
		DO J = 0, TS_NUMBS - 1
			IF (MOD(I+J,2)==0) THEN
				TS_CTL%WGTS(I,J,0) = 1.D0
			ELSE
				TS_CTL%WGTS(I,J,0) = DCOS(45.D0*DEGREE)
			ENDIF
		ENDDO
	ENDDO
	
! 	CALL FIND_CTL_WGT_BERNSTEIN(TS_CTL, (/2,2/), 'X')
	
! IGA - L-shaped domain with multiple patches (clockwise order)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! 	
! 	GEO_CTL(1)%PTS(0,1,0) = POINT2D(0.D0,0.5D0)
! 	GEO_CTL(1)%PTS(1,1,0) = POINT2D(0.5D0*DCOS(45.D0*DEGREE),0.5D0*DSIN(45.D0*DEGREE))
! 	GEO_CTL(1)%PTS(2,1,0) = POINT2D(1.D0,-1.D0 + DCOS(45.D0*DEGREE) + DSIN(45.D0*DEGREE))
! 	
! 	GEO_CTL(1)%PTS(0,2,0) = POINT2D(0.D0,1.D0)
! 	GEO_CTL(1)%PTS(1,2,0) = POINT2D(GEO_CTL(1)%PTS(2,1,0)%Y, GEO_CTL(1)%PTS(2,1,0)%X)
! 	GEO_CTL(1)%PTS(2,2,0) = POINT2D(DCOS(45.D0*DEGREE), DSIN(45.D0*DEGREE))
! 	
! 	GEO_CTL(1)%WGTS(0,0,0) = 1.D0
! 	GEO_CTL(1)%WGTS(1,0,0) = 1.D0
! 	GEO_CTL(1)%WGTS(2,0,0) = 1.D0
! 	
! 	GEO_CTL(1)%WGTS(0,1,0) = 1.D0
! 	GEO_CTL(1)%WGTS(1,1,0) = 1.D0
! 	GEO_CTL(1)%WGTS(2,1,0) = DCOS(22.5D0*DEGREE)
! 	
! 	GEO_CTL(1)%WGTS(0,2,0) = 1.D0
! 	GEO_CTL(1)%WGTS(1,2,0) = DCOS(22.5D0*DEGREE)
...
! 	GEO_CTL(2)%PTS(2,0,0) = POINT2D(0.D0,0.D0)
! 	
! 	GEO_CTL(2)%PTS(0,1,0) = POINT2D(-1.D0,-1.D0 + DCOS(45.D0*DEGREE) + DSIN(45.D0*DEGREE))
! 	GEO_CTL(2)%PTS(1,1,0) = POINT2D(-0.5D0*DCOS(45.D0*DEGREE),0.5D0*DSIN(45.D0*DEGREE))
! 	GEO_CTL(2)%PTS(2,1,0) = POINT2D(0.D0,0.5D0)
! 	
! 	GEO_CTL(2)%PTS(0,2,0) = POINT2D(-DCOS(45.D0*DEGREE), DSIN(45.D0*DEGREE))
! 	GEO_CTL(2)%PTS(1,2,0) = POINT2D(-(-1.D0 + DCOS(45.D0*DEGREE) + DSIN(45.D0*DEGREE)), 1.D0)
! 	GEO_CTL(2)%PTS(2,2,0) = POINT2D(0.D0,1.D0)
! 	
! 	GEO_CTL(2)%WGTS(0,0,0) = 1.D0
! 	GEO_CTL(2)%WGTS(1,0,0) = 1.D0
! 	GEO_CTL(2)%WGTS(2,0,0) = 1.D0
! 	
! 	GEO_CTL(2)%WGTS(0,1,0) = DCOS(22.5D0*DEGREE)
! 	GEO_CTL(2)%WGTS(1,1,0) = 1.D0
! 	GEO_CTL(2)%WGTS(2,1,0) = 1.D0
! 	
! 	GEO_CTL(2)%WGTS(0,2,0) = 1.D0
! 	GEO_CTL(2)%WGTS(1,2,0) = DCOS(22.5D0*DEGREE)
! 	GEO_CTL(2)%WGTS(2,2,0) = 1.D0
! 	
! 	
! 	!! IGA PATCH #3
! 	GEO_CTL(3)%PTS(0,0,0) = POINT2D(-DCOS(45.D0*DEGREE), -DSIN(45.D0*DEGREE))
! 	GEO_CTL(3)%PTS(1,0,0) = POINT2D(-(-1.D0 + DCOS(45.D0*DEGREE) + DSIN(45.D0*DEGREE)), -1.D0)
! 	GEO_CTL(3)%PTS(2,0,0) = POINT2D(0.D0,-1.D0)
! 	
! 	GEO_CTL(3)%PTS(0,1,0) = POINT2D(-1.D0,-(-1.D0 + DCOS(45.D0*DEGREE) + DSIN(45.D0*DEGREE)))
! 	GEO_CTL(3)%PTS(1,1,0) = POINT2D(-0.5D0*DCOS(45.D0*DEGREE),-0.5D0*DSIN(45.D0*DEGREE))
! 	GEO_CTL(3)%PTS(2,1,0) = POINT2D(0.D0,-0.5D0)
! 	
! 	GEO_CTL(3)%PTS(0,2,0) = POINT2D(-1.D0,0.D0)
! 	GEO_CTL(3)%PTS(1,2,0) = POINT2D(-0.5D0,0.D0)
! 	GEO_CTL(3)%PTS(2,2,0) = POINT2D(0.D0,0.D0)
! 	
! 	GEO_CTL(3)%WGTS(0,0,0) = 1.D0
! 	GEO_CTL(3)%WGTS(1,0,0) = DCOS(22.5D0*DEGREE)
! 	GEO_CTL(3)%WGTS(2,0,0) = 1.D0
! 	
! 	GEO_CTL(3)%WGTS(0,1,0) = DCOS(22.5D0*DEGREE)
! 	GEO_CTL(3)%WGTS(1,1,0) = 1.D0
! 	GEO_CTL(3)%WGTS(2,1,0) = 1.D0
! 	
! 	GEO_CTL(3)%WGTS(0,2,0) = 1.D0
! 	GEO_CTL(3)%WGTS(1,2,0) = 1.D0
! 	GEO_CTL(3)%WGTS(2,2,0) = 1.D0
	
	!! IGA PATCH #4
! 	GEO_CTL(4)%PTS(0,0,0) = POINT2D(0.D0,-1.D0)
! 	GEO_CTL(4)%PTS(1,0,0) = POINT2D((-1.D0 + DCOS(45.D0*DEGREE) + DSIN(45.D0*DEGREE)), -1.D0)
! 	GEO_CTL(4)%PTS(2,0,0) = POINT2D(DCOS(45.D0*DEGREE), -DSIN(45.D0*DEGREE))
! 	
! 	GEO_CTL(4)%PTS(0,1,0) = POINT2D(0.D0,-0.5D0)
! 	GEO_CTL(4)%PTS(1,1,0) = POINT2D(0.5D0*DCOS(45.D0*DEGREE),-0.5D0*DSIN(45.D0*DEGREE))
! 	GEO_CTL(4)%PTS(2,1,0) = POINT2D(1.D0,-(-1.D0 + DCOS(45.D0*DEGREE) + DSIN(45.D0*DEGREE)))
! 	
! 	GEO_CTL(4)%PTS(0,2,0) = POINT2D(0.D0,0.D0)
! 	GEO_CTL(4)%PTS(1,2,0) = POINT2D(0.5D0,0.D0)
! 	GEO_CTL(4)%PTS(2,2,0) = POINT2D(1.D0,0.D0)
! 	
! 	GEO_CTL(4)%WGTS(0,0,0) = 1.D0
! 	GEO_CTL(4)%WGTS(1,0,0) = DCOS(22.5D0*DEGREE)
! 	GEO_CTL(4)%WGTS(2,0,0) = 1.D0
! 	
! 	GEO_CTL(4)%WGTS(0,1,0) = 1.D0
! 	GEO_CTL(4)%WGTS(1,1,0) = 1.D0
! 	GEO_CTL(4)%WGTS(2,1,0) = DCOS(22.5D0*DEGREE)
! 	
! 	GEO_CTL(4)%WGTS(0,2,0) = 1.D0
! 	GEO_CTL(4)%WGTS(1,2,0) = 1.D0
! 	GEO_CTL(4)%WGTS(2,2,0) = 1.D0
...
! Refine knot vector inserting new knots with multiplicities are greater than 1
	...
	! 		PRINT*, 'DIR', DIR
			IF (EXTRA_KNOTS(DIR).GT.0) THEN
				IGA_KVEC(PATCH,DIR) = UNIFORM_KNOT_INSERTION(IGA_KVEC(PATCH,DIR), EXTRA_KNOTS(DIR))
...
! Local Index
	ALLOCATE(INTERFACE_NDX(NUMBS(1,2) + NUMBS(2,1) - 1))
