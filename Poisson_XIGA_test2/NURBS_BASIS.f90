...
SUBROUTINE GET_ALL_PHY_NURBS_SURFACE_2D(SF, INDX, REF_PT, JACOB, PATCH, INV)
	...
	TYPE(FVALUE), INTENT(OUT) :: SF((GEO_KVEC(PATCH,1)%POLY_ORDER+1)*(GEO_KVEC(PATCH,2)%POLY_ORDER+1))
	TYPE(INT2D), INTENT(OUT) :: INDX((GEO_KVEC(PATCH,1)%POLY_ORDER+1)*(GEO_KVEC(PATCH,2)%POLY_ORDER+1))
...
! 	ENDIF
	...
	DBSFUN(1) =  GET_ALL_DIFF_BSPLINES(REF_PT%X,GEO_KVEC(PATCH,1),DIFF_ORDER)
	DBSFUN(2) =  GET_ALL_DIFF_BSPLINES(REF_PT%Y,GEO_KVEC(PATCH,2),DIFF_ORDER)

	DO K = 0, DIFF_ORDER
	DO L = 0, DIFF_ORDER-K
		DWSUM(K,L) = 0.0D0
		DO J1 = 0, GEO_KVEC(PATCH,1)%POLY_ORDER
			INDX_X = DBSFUN(1)%INIT + J1
			IF (INDX_X>=0) THEN
				DO J2 = 0, GEO_KVEC(PATCH,2)%POLY_ORDER
					INDX_Y = DBSFUN(2)%INIT + J2
					IF (INDX_Y>=0) THEN
						DWSUM(K,L) = DWSUM(K,L) + DBSFUN(1)%N(J1,K)*DBSFUN(2)%N(J2,L)*GEO_CTL(PATCH)%WGTS(INDX_X,INDX_Y,0)
					ENDIF
				ENDDO
			ENDIF
		ENDDO
	...
	DO I1 = 0, GEO_KVEC(PATCH,1)%POLY_ORDER
		
		INDX_X = DBSFUN(1)%INIT + I1

	DO I2 = 0, GEO_KVEC(PATCH,2)%POLY_ORDER
		...
		DO L = 0, DIFF_ORDER-K
			...
			DIFF_NURBS%VAL(K,L) = DBSFUN(1)%N(I1,K)*DBSFUN(2)%N(I2,L)*GEO_CTL(PATCH)%WGTS(INDX_X,INDX_Y,0)
