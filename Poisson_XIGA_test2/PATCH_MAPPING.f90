...
! 		ENDIF
	...
	TYPE(POINT2D) FUNCTION GET_REF_PT(PHY_PT, PATCH)
...
! 				PRINT*, 'GET_REF_PT-HERE2'
				IF (PATCH.NE.1 .AND. (OLD_PT%X.LE.0.D0 .OR. OLD_PT%X.GE.1.D0)) THEN
					IF (PHY_PT%X.LE.EPS) THEN
						OLD_PT%X = 0.1D0*K
					ELSE
						OLD_PT%X = 0.5D0 + 0.01D0*K
					ENDIF
					ITER = 0
				ENDIF
				IF (PATCH.NE.1 .AND. (OLD_PT%Y.LE.0.D0 .OR. OLD_PT%Y.GE.1.D0)) THEN
					IF (DSQRT(PHY_PT%X**2 + PHY_PT%Y**2).LE.0.5D0) THEN
						OLD_PT%Y = 0.1D0*K
					ELSE
						OLD_PT%Y = 0.9D0 + 0.01D0*K
					ENDIF
				ENDIF
			ENDIF
		ENDDO
