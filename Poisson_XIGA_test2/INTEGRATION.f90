...
SUBROUTINE GEN_BD_LN(SUB_K, SUB_F)
	...
	INTEGER :: I, J, II, JJ, KK, N, GLOBAL_INDX(2), PATCH, SUB_NUMLN
	TYPE(VEC2D) :: SUB_LN
	TYPE(MATRIX_22) :: JACOB
...
! 			PRINT*, 'RIGHT', PATCH, JJ, SUB_F(11)
	...
	ALLOCATE(SF1(NUMGSPT, (BASIS_KVEC(1,1)%POLY_ORDER+1)*(BASIS_KVEC(1,2)%POLY_ORDER+1)))
	ALLOCATE(INDX1((BASIS_KVEC(1,1)%POLY_ORDER+1)*(BASIS_KVEC(1,2)%POLY_ORDER+1)))
	
	ALLOCATE(SF2(NUMGSPT, (BASIS_KVEC(2,1)%POLY_ORDER+1)*(BASIS_KVEC(2,2)%POLY_ORDER+1)))
	ALLOCATE(INDX2((BASIS_KVEC(2,1)%POLY_ORDER+1)*(BASIS_KVEC(2,2)%POLY_ORDER+1)))
	...
	DO JJ = 1, NUMIR(PATCH,2)-1
		...
		DO II=1, SUB_NUMLN
			SUB_LN = VEC2D(POINT2D(0.D0,IR_GRID(PATCH,2,JJ)+(II-1)*SUB_LN_LENGTH), POINT2D(0.D0,IR_GRID(PATCH,2,JJ)+II*SUB_LN_LENGTH))
			DIFF_NN = 0.D0
			CALL LEAST_SQUARE_LC_INT_IJ(SF1, SF2, INDX1, INDX2, WEIGHT, (/1,PATCH/), SUB_LN, 'L')
			DO I=1, UBOUND(INDX1,1)
				DO J=1, UBOUND(INDX2,1)
					CALL DR_GL_INDX_FIJ(GLOBAL_INDX, (/1,2/), (/I,J/), INDX1, INDX2)
					IF (GLOBAL_INDX(1).EQ.0 .OR. GLOBAL_INDX(2).EQ.0) THEN
					ELSE
		...
		DO II=1, SUB_NUMLN
			SUB_LN = VEC2D(POINT2D(1.D0,IR_GRID(PATCH,2,JJ)+(II-1)*SUB_LN_LENGTH), POINT2D(1.D0,IR_GRID(PATCH,2,JJ)+II*SUB_LN_LENGTH))
			DIFF_NN = 0.D0
			CALL LEAST_SQUARE_LC_INT_IJ(SF1, SF2, INDX1, INDX2, WEIGHT, (/1,PATCH/), SUB_LN, 'R')
			DO I=1, UBOUND(INDX1,1)
				DO J=1, UBOUND(INDX2,1)
					CALL DR_GL_INDX_FIJ(GLOBAL_INDX, (/1,2/), (/I,J/), INDX1, INDX2)
					IF (GLOBAL_INDX(1).EQ.0 .OR. GLOBAL_INDX(2).EQ.0) THEN
...
SUBROUTINE STIF_LC_INT_IJ(SF1, SF2, INDX1, INDX2, WEIGHT, PATCHES, NUMIR_INDX)
	...
	TYPE(MATRIX_22) :: JACOB_F, JACOB_FG, INV_JACOB_F, JACOB_G, INV_JACOB_F_SQRT
	INTEGER :: I,J, K, II, JJ, KK
	REAL*8 :: DET_JF
...
! 			PRINT*, INV_PHYPT_G
			...
			INV_JACOB_F_SQRT%ENT(1,1) = DOT_PRODUCT(INV_JACOB_F%ENT(1,:), INV_JACOB_F%ENT(:,1))
			INV_JACOB_F_SQRT%ENT(1,2) = DOT_PRODUCT(INV_JACOB_F%ENT(1,:), INV_JACOB_F%ENT(:,2))
			INV_JACOB_F_SQRT%ENT(2,1) = DOT_PRODUCT(INV_JACOB_F%ENT(2,:), INV_JACOB_F%ENT(:,1))
			INV_JACOB_F_SQRT%ENT(2,2) = DOT_PRODUCT(INV_JACOB_F%ENT(2,:), INV_JACOB_F%ENT(:,2))
			
			JACOB_FG%ENT(1,1) = DOT_PRODUCT(INV_JACOB_F_SQRT%ENT(1,:), JACOB_G%ENT(:,1))
			JACOB_FG%ENT(1,2) = DOT_PRODUCT(INV_JACOB_F_SQRT%ENT(1,:), JACOB_G%ENT(:,2))
			JACOB_FG%ENT(2,1) = DOT_PRODUCT(INV_JACOB_F_SQRT%ENT(2,:), JACOB_G%ENT(:,1))
			JACOB_FG%ENT(2,2) = DOT_PRODUCT(INV_JACOB_F_SQRT%ENT(2,:), JACOB_G%ENT(:,2))
			
			DET_JF = .DETERMINANT.JACOB_F
			WEIGHT(KK) = DET_JF*GSXW(I)*GSYW(J)
! 			PRINT*, 'STIF_LC_INT_IN - HERE0'
			CALL GET_ALL_PHY_NURBS_SURFACE_2D(SF1(KK,:), INDX1(:), INV_PHYPT_G, JACOB_FG, PATCHES(1), 'INV')
...
SUBROUTINE STIF_FIND_GLOBAL_INDX_FIJ(GLOBAL_INDX, PATCHES, LOCAL_INDX, INDX1, INDX2)
	...
	TYPE(INT2D), INTENT(IN) :: INDX1((BASIS_KVEC(1,1)%POLY_ORDER+1)*(BASIS_KVEC(1,2)%POLY_ORDER+1))
	TYPE(INT2D), INTENT(IN) :: INDX2((BASIS_KVEC(2,1)%POLY_ORDER+1)*(BASIS_KVEC(2,2)%POLY_ORDER+1))
	INTEGER, INTENT(OUT) :: GLOBAL_INDX(2)
